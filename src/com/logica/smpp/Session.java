 ...
 * with SMSC using SMPP protocol, i.e. methods for sending PDUs as defined in
 * SMPP specification as well as receiving responses for sent PDUs and waiting
 * for PDUs whose sending was initiated by SMSC.<br>
 * Instance of <code>Session</code> represents one connection of ESME to a SMSC.
 * Multiple connections can be established using multiple <code>Sessions</code>.
 * <p>
 * <code>Session</code> uses <code>Connection</code> object which is
 * instantiated outside of the <code>Session</code>. This way is the
 * <code>Session</code> made independent on the communication protocol and
 * <code>Session</code>'s code isn't populated by protocol dependent
 * initialisation.
 * <p>
 * Code example of binding, sending one message and unbinding: <br>
 * <blockquote>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * </blockquote> Note that the cycle bind - send PDU's - unbind can be called
 * several times for once created session.
 * <p>
 * Particular methods for sending PDUs to SMSC return responses to the sent
 * PDUs. They return null if no response is received in time specified by the
 * receive timeout in receiver. This means that the methods wait for response
 * corresponding to the request. The corresponding response is recognized using
 * sequence number of the sent PDU and a corresponding response command id.<br>
 * The session can work in assynchronous manner, i.e. it doesn't wait for
 * response for the sent request, instead all responses are handled by instance
 * of callback class <code>ServerPDUEventListener</code> whenever they are
 * received.<br>
 * The <code>Session</code> class checks if operations invoked are valid in the
 * current state of the session. If not, then such operation throws
 ...
 * if the operation is valid in the current state of session is turned off by
 * default.
 * 
...
/*
 * 08-08-01 ticp@logica.com added asynchronous processing capability 02-10-01
 * ticp@logica.com tracing now belongs to DSESS group 20-11-01 ticp@logica.com
 * the receiver thread is now started even for the transmitter session - before
 * transmitter sessions couldn't receive anything 20-11-01 ticp@logica.com
 * receiver thread is started after the bind request is sent to the MC, before
 * it was started before the bind req was sent to MC, therefore the response
 * could be processed by the listener 22-11-01 ticp@logica.com implemented
 * session states with checking if certain operation is allowed in the current
 * session state
 */
...
{
	/**
	 * Status of the connection. It's set by <code>open</code> method, which is
	 * called from <code>bind</code> method.
	 * 
	 * @see #open()
	...
	/**
	 * Status of bounding. It's set by <code>bind</code> methdod if the binding
	 * is successfull.
	 * 
	 * @see #bind(BindRequest)
	 */
	...
	/**
	 * Table with command id's and session states in which they can be used. For
	 * session on ESME side.
	 * 
	...
	/**
	 * Table with command id's and session states in which they can be used. For
	 * session on MC side. Note that even if this is from the MC side the bind
	 * states are still entered from the EMSE point of veiw. E.g. for deliver_sm
	 * this table contains states receiver and transceiver, which means that MC
	 * can send PDU with deliver_sm command id only in case the ESME is bound as
	 * receiver or transceiver.<br>
	 * <emp>Note:</emp> The MC state matrix is not completly supported by the
	 * current implementation of session, so it's currently altered to reflect
	 * the impplementation.
	 * 
	 * @see #initialiseStateMatrix()
	 */
	...
	/**
	 * If PDU with unknown command id is probed this says if this PDU is allowed
	 * or not. Generaly it's not good idea to allow unknown PDUs, but for
	 * backward compatiblity we leave it set to false.
	 */
	 ...
	 * set to true, it'll save you communication bandwidth (MC should reject
	 * your PDU if you are in wrong state) and it'll discover bad logic in your
	 * application. For backward compatibility we set that the state is NOT
	 * checked.
	 * 
	...
	/**
	 * Indicates that the session is session for MC 'point of view.' MC is
	 * Message Center and it is a term for generic Message Centre such as SMSC.
	 * 
	 * @see #type
	 */
	...
	/**
	 * If this session is ESME session or if it is used in MC (or simulator or
	 * routing entity, simply the other side of normal ESME). Normally this
	 * would be equal to TYPE_ESME as mostly this library will be used for
	 * developing ESME applications and not MCs. If you don't understand what is
	 * this variable for, just leave it as it is.
	 * 
	 * @see #setType(int)
	 * @see #getType()
	 */
	 ...
	 * class and passed as a parameter during the <code>Session</code> creation.
	 * It's then passed to <code>Transmitter</code> and <code>Receiver</code>.
	 * 
	 * @see Connection
	 * @see TCPIPConnection
	 * @see Transmitter
	...
	/**
	 * If the receiving is asynchronous, <code>pduListener</code> must contain
	 * the callback object used for processing of PDUs received from the SMSC.
	 * <code>Receiver</code> after receiving a PDU passes the received PDU to
	 * apropriate member function of the processor.
	 * 
	 * @see #asynchronous
	...
	/**
	 * Indicates that the sending of PDUs to the SMSC is asynchronous, i.e. the
	 * session doesn't wait for a response to the sent request as well as the
	 * <code>receive</code> functions will return null as all received PDUs are
	 * passed to the <code>pduListener</code> object in the
	 * <code>receiver</code>.
	 * 
	 * @see #pduListener
	 * @see #setServerPDUEventListener(ServerPDUEventListener)
	...
	/**
	 * Default constructor made protected as it's not desirable to allow
	 * creation of <code>Session</code> without providing
	 ...
	 *
	 * @param connection
	 *            connection used for transmitting and receiving the data
	 */
	...
	/**
	 * Opens the connection for communication. Sets indication that the
	...
	/**
	 * Closes the connection for communication. Sets indication that the
	 ...
	 * <code>Receiver</code> from the SMSC. Note that this method implicitly
	 * sets asynchronous type of receiving.
	 * 
	 * @param pduListener
	 *            the listener which processes the received PDUs
	 */
	...
	/**
	 * Returns the current <code>ServerPDUEventListener</code> set for this
	 * session.
	 */
	 ...
	 * from the side where the instance of the <code>Session</code> is used. Set
	 * the type of the session before opening the session.
	 * 
	 * @param type
	...
	/**
	 * Opens connection and binds to SMSC using the bind method provided as
	 * bindReq parameter. Binds synchronously as no server pdu listener is
	 * provided. On details about bind see
	 * <a href="#bind(BindRequest,ServerPDUEventListener)">bind(BindRequest,
	 * ServerPDUEventListener)</a>.
	 * 
	...
	/**
	 * Opens connection and binds to SMSC using the bind method provided as
	 * <code>bindReq</code> parameter. For transmittong PDUs creates instance of
	 * <code>Transmitter</code> class, for receiving PDUs from SMSC creates
	 * instance of <code>Receiver</code> class.<br>
	 * Receiver starts an extra thread which receives PDUs from connection and
	 * puts them into a queue (if synchronous) or processes them using
	 * <code>pduListener</code>. If the bind to SMSC isn't successfull, the
	 * thread is stopped and the connection is closed.<br>
	 * If the variable <code>pduListener</code> is not <code>null</code>, the
	 * session is asynchronous, otherwise the session is synchronous. Note that
	 * the <code>bind</code> method is <emp>always</emp> synchronous, regardless
	 * of the value of the <code>pduListener</code> variable.
	 * <p>
	 ...
	 *                rest of data not received for too long time
	...
	/**
	 * Sends an outbind PDU over the connection. For sending outbind PDU the
	 * session must not be bound to smsc (!), this is the logic of outbind
	 * operation. Don't mess it up with receiving outbind PDUs from SMSC! Also
	 * this operation is only allowed when your session acts as SMSC (MC)
	 * session. ESMEs can't send outbinds.
	 * <p>
	 ...
	 *                rest of data not received for too long time
	 * @exception ValueNotSetException
	 *                optional param not set but requested (this exception
	 *                doesn't apply for outbind)
	 * @see Outbind
	 * @see OutbindReceiver
	 */
	...
	/**
	 * Stops <code>receiver</code> (if applicable), unbinds from SMSC by sending
	 * <code>Unbind</code> PDU, waits for <code>UnbindResp</code> and then
			...
			throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException
	{
		...
		if (bound)
		{
			...
			if (asynchronous)
			{
				...
				UnbindServerPDUEventListener unbindListener = new UnbindServerPDUEventListener(this, origListener,
						unbindReq);
	...
	/**
	 * Creates and sends <code>GenericNack</code> PDU with command status and
	 * sequence number provided as parameters.
	 * <p>
	 * See "SMPP Protocol Specification 3.4, 4.3 GENERIC_NACK Operation."
	 *
	 * @param commandStatus
	 *            the code of error to be reported
	 * @param sequenceNumber
	 *            the sequence number of the PDU the GenericNack is related to
	 * @exception IOException
	 *                exception during communication
	 * @exception TimeoutException
	 *                rest of data not received for too long time
	 ...
	 *
	 * @param request
	 *            the submit multi pdu to be submitted
	 * @return correspondent response pdu
	 *
	 * @exception IOException
	 *                exception during communication
	...
	/**
	 * Submits provided <code>DeliverSM</code> PDU to SMSC and returns response
	 * to the submission.
	 * <p>
	 * See "SMPP Protocol Specification 3.4, 4.6 DELIVER_SM Operation."
	 *
	 * @param request
	 *            the deliver pdu to be submitted
	 * @return correspondent response pdu
	 *
	 * @exception IOException
	 *                exception during communication
	...
	/**
	 * Submits provided <code>DataSM</code> PDU to SMSC and returns response to
	 * the submission.
	 * <p>
	 * See "SMPP Protocol Specification 3.4, 4.7 DATA_SM Operation."
	 *
	 * @param request
	 *            the data pdu to be submitted
	 * @return correspondent response pdu
	 *
	 * @exception IOException
	 *                exception during communication
	...
	/**
	 * Queries status of previous submission by sending the <code>QuerySM</code>
	 * PDU to SMSC; returns the query response.
	 * <p>
	 * See "SMPP Protocol Specification 3.4, 4.8 QUERY_SM Operation."
	 *
	 * @param request
	 *            the status query pdu to be sent
	 * @return correspondent response pdu
	 *
	 * @exception IOException
	 *                exception during communication
	 ...
	 * <code>EnquireLink</code> PDU to SMSC; returns response to the enquiry.
	 * <p>
	 * See "SMPP Protocol Specification 3.4, 4.11 ENQUIRE_LINK Operation."
	 *
	 * @param request
	 *            the enquiry pdu to be submitted
	 * @return correspondent response pdu
	 *
	 * @exception IOException
	 *                exception during communication
	 ...
	 * axcept of header, there is might be no need to provide the PDU as a
	 * parameter to the <code>enquireLink</code> method.<br>
	 * This method creates new <code>EnquireLink</code> object and sends it to
	 * SMSC; returns response to the enquiry.
	 * <p>
	 * See "SMPP Protocol Specification 3.4, 4.11 ENQUIRE_LINK Operation."
	 *
	 * @return correspondent response pdu
	...
	/**
	 * Returns a PDU received from SMSC. This is blocking receive, caller will
	 * wait until a PDU will be received.<br>
	 * Note that this method can be called only when bound as receiver or
	 * transciever.
	 *
	 * @return received pdu
	 * 
	 * @exception IOException
	 *                exception during communication
	 * @exception NotSynchronousException
	 *                receive called in asynchronous mode
	 * @exception PDUException
	 *                incorrect format of PDU
	 * @exception TimeoutException
	 *                rest of data not received for too long time
	...
	/**
	 * Returns a PDU received from SMSC. This receive will wait for maximum
	 * <code>timeout</code> time for a PDU; if there is no PDU received in the
	 * specified time, the function returns null.<br>
	 * Note that this method can be called only when bound as receiver or
	 * transciever.
	 *
	...
	/**
	 * Sends a response PDU. Use for sending responses for PDUs send from SMSC,
	 * e.g. DELIVERY_SM etc.
	 *
	 * @param response
	...
	/**
	 * Returns <code>Transmitter</code> object created for transmitting PDUs to
	 * SMSC.
	 *
	...
	/**
	 * Returns <code>Receiver</code> object created for receiving PDUs from
	 * SMSC.
	 *
	...
	/**
	 * Returns <code>Connection</code> object provided for communication with
	 * SMSC.
	 *
	...
	/**
	 * The basic method for sending a <code>Request</code> PDU to SMSC and
	 * receiving correspondent <code>Response</code>, if applicable, and returns
	 * the received PDU.<br>
	 ...
	 * to report the error and null is returned. If the timeout for receiving
	 * PDU set in the <code>Receiver</code> object expires, null is returned.
	 * <p>
	 * 
	 * @param request
	 *            the request PDU (not xxx_RESP) to be sent
	...
	/**
	 * Checks if the <code>pdu</code> received is matching the
	 * <code>expResponse</code> and returns corrected response. If the command
	 * id's don't match then if the <code>pdu</code>'s command id is
	 * generic_nack, then the expected response is set the command id of
	 * generic_nack and command id, command status and sequence number of the
	 * received <code>pdu</code> and this way transformed response is returned:
	 * the class mathces, but the command id says, that the PDU is generic nack.
	 * <br>
	 * If the command id is not generic nack, then we received incorrect pdu as
	 * the sequence numbers match, so we send generic nack back to the smsc. if
	 * the command id's match, then the <code>pdu</code> is returned.
	 * 
	 * @param pdu
	 *            the received PDU which should be a response
	 * @param expResponse
	 *            the response we expected from smsc
			...
			throws ValueNotSetException, TimeoutException, IOException
	{
		...
		if (pdu.getCommandId() != expResponse.getCommandId())
		{
			...
			else
			{
				...
				safeGenericNack(Data.ESME_RINVCMDID, response.getSequenceNumber());
				response = null;
			}
		}
		else
		{
			// if the commandId is same as of the expected response's command
			// id,
	 ...
	 * 
	 * @param commandStatus
	 *            command status to report
	 * @param sequenceNumber
	 *            to allow the other party to match the generic ack with their
	 *            sent request
	 * @exception IOException
	 *                if there is a comms error
	 */
	...
	/**
	 * Sets the state of the session. It's private as it's only for use by the
	...
	/**
	 * Returns the current state of the session. It's public to allow the user
	 * to check the state of the session if necessary.
	 * 
	...
	/**
	 * Enables checking if the session allows certain operation in the current
	 * state.
	 */
	...
	/**
	 * Disables checking if the session allows certain operation in the current
	 * state.
	 */
	 ...
	 *            the state(s) in which the session is expected to be; if it is
	 *            not, then exception is thrown
	 * @throws WrongSessionStateException
	 *             if the session is not in the state required
	 * @see WrongSessionStateException
	 */
	public void checkState(int requestedState) throws WrongSessionStateException
	{
		if (stateChecking)
		{
			debug.write(DSESS, "checking state current=0x" + Integer.toHexString(state) + " requested esme=0x"
	...
	/**
	 * Checks if the session's state allows sending the PDU provided. If not,
	 * then exception <code>WrongSessionStateException</code> is thrown. For
	 * each state there is only a subset of PDUs which can be sent over the
	 * session. For example, if the session is bound as a receiver, it cannot
	 * submit messages, but it can send enquire link, generic nack etc. This
	 * method checks the PDU type (command id) aganst matrix of allowed states.
	 * The checking can be turned off, see
	 * <a href="#disableStateChecking()">disableStateChecking</a>.
	 * 
	 * @param pdu
	 *            the pdu which has to be checked if it's allowed in the current
	 *            state
	 * @throws WrongSessionStateException
	 *             if the session is not in the state required
	 * @see #checkState(int)
	 ...
	 * 
	 * @param requestedState
	 *            the state(s) which have to be checked
	...
	/**
	 * Checks if the pdu provided is allowed in the current session state. Note
	 * that this method doesn't throw an exception rather it returns false if
	 * the pdu is not alloewd in the current session state.
	 * 
	 * @param pdu
	 *            the pdu which has to be checked if it's allowed in the current
	 *            state
	...
	/**
	 * Initialises state matrices for checking if PDU is allowed in a certain
	 * session state.
	 */
	static
	{
	...
	private static void initialiseStateMatrix()
	{
		...
		addValidState(esmeStateMatrix, Data.ENQUIRE_LINK, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_ALWAYS);
		addValidState(esmeStateMatrix, Data.ENQUIRE_LINK_RESP, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_ALWAYS);
		addValidState(esmeStateMatrix, Data.ALERT_NOTIFICATION, STATE_NOT_ALLOWED);
		addValidState(esmeStateMatrix, Data.GENERIC_NACK, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_ALWAYS);

		mcStateMatrix = new Hashtable();
		addValidState(mcStateMatrix, Data.BIND_TRANSMITTER, STATE_NOT_ALLOWED);
		addValidState(mcStateMatrix, Data.BIND_TRANSMITTER_RESP,
				STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_OPENED);
		addValidState(mcStateMatrix, Data.BIND_RECEIVER, STATE_NOT_ALLOWED);
		addValidState(mcStateMatrix, Data.BIND_RECEIVER_RESP, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_OPENED);
		addValidState(mcStateMatrix, Data.BIND_TRANSCEIVER, STATE_NOT_ALLOWED);
		addValidState(mcStateMatrix, Data.BIND_TRANSCEIVER_RESP,
				STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_OPENED);
		addValidState(mcStateMatrix, Data.OUTBIND, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_OPENED);
		...
		addValidState(mcStateMatrix, Data.ENQUIRE_LINK, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_ALWAYS);
		addValidState(mcStateMatrix, Data.ENQUIRE_LINK_RESP, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_ALWAYS);
		addValidState(mcStateMatrix, Data.ALERT_NOTIFICATION, STATE_RECEIVER | STATE_TRANSCEIVER);
		addValidState(mcStateMatrix, Data.GENERIC_NACK, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_ALWAYS);
	}
	 ...
	 * 
	 * @param matrix
	 *            the matrix to add the mapping to
	 * @param commandId
	 *            the commandId of the PDU the mapping is created for
	 * @param state
	 *            the state(s) in which is the PDU valid
	 * @see #checkState(PDU)
	...
	/**
	 * God, I would never think that to keep unbind synchronous in an
	 * asynchronous enviroment would be so funny. Here is the replacement
	 ...
	 * even if the session is asynchronous, i.e. all pdus from the smsc are
	 * passed to an implementation of <code>ServerPDUEventListener</code> event
	 * responses. We can't simply stop the asynchronicity as there can still be
	 * some responses expected, so we need a bridge which allows us to wait for
	 * the unbind response and still serve the other pdus from the smsc in
	 * asynchronous manner. Thus this encapsulating listener, which exactly does
	 * the thing.
	 */
			...
			if (pdu.equals(unbindReq))
			{
				synchronized (this)
				{
					...
					catch (Exception e)
					{
						...
						super.event.write(e, "exception handling unbind");
					}
