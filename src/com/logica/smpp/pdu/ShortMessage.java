...
/**
 * Provides encapsulation of message data with optional message encoding. Can
 * contain an ordinary data message or a message containing data encoded in one
 * of the Java supported encodings, including multibyte. On Java encodings see
...
{
	/**
	 * Minimal size of the message in bytes. For multibyte encoded messages it
	 * means size after converting to sequence of octets.
	 */
	int minLength = 0;

	/**
	 * Max size of the message in octets. For multibyte encoded messages it
	 * means size after converting to sequence of octets.
	 */
	int maxLength = 0;
	...
	/**
	 * The message data after conversion to the sequence of octets, i.e. the
	 * octets.
	 */
	byte[] messageData = null;

	/**
	 * Construct the short message with max data length -- the max count of
	 * octets carried by the massege. It's not count of chars when interpreted
	...
	/**
	 * Construct the short message with mina nd max data length -- the min and
	 * max count of octets carried by the massege. It's not count of chars when
	 * interpreted with certain encoding.
	 * 
	 * @param minLength
	 ...
	 * 
	 * @param buffer
	 *            the buffer containing the message data; must contain exactly
	 *            the data of the message (not zero terminated nor length
	 *            tagged)
	 * @see #getMessage()
	...
	/**
	 * Returns the sequence of octets generated from the message according the
	 * encoding provided.
	 * 
	...
	/**
	 * Sets the message a new value. Default encoding
	 * <code>Data.ENC_ASCII</code> is used.
	 * 
	 * @param message
	 *            the message
	 * @exception WrongLengthOfStringException
	 *                thrown when the message too short or long
	 */
	 ...
	 *            the message
	 * @param encoding
	 *            the encoding of the message provided
	 * @exception WrongLengthOfStringException
	 *                thrown when the message too short or long
	 * @exception UnsupportedEncodingException
	 *                if the required encoding is not available for the Java
	 *                Runtime system
	 */
	...
	/**
	 * Sets the encoding of the messasge. Handy for message read from
	 * <code>ByteBuffer</code> to set the encoding ad hoc.
	 * 
	 * @param encoding
	 ...
	 * 
	 * @param encoding
	 *            the required encoding of the resulting (String) message
	 * @exception UnsupportedEncodingException
	 *                if the required encoding is not available for the Java
	 *                Runtime system
	 */
	...
	public static boolean encodingSuported(String encoding)
	{
		...
		/*
		 * try { CharToByteConverter.getConverter(encoding); } catch
		 * (UnsupportedEncodingException e) { supported = false; }
		 */
		return supported;
	}
