...
public abstract class PDU extends ByteData
{
        ...
        try {
            ...
            if (buffer.length()<Data.PDU_HEADER_SIZE) {
                if (debug.active(DPDU)) {
                    debug.write(DPDU,"PDU.setData() not enough data for header in the buffer " +
                                buffer.getHexDump());
                }
            }
            ...
            if (debug.active(DPDU)) {
                debug.write(DPDU,"PDU.setData() parsing header " +
                            headerBuf.getHexDump());
            }
            setHeader(headerBuf);
            ...
            if ((initialBufLen - buffer.length()) < getCommandLength()) {
                ...
                int optionalLength =
                    getCommandLength() + buffer.length() - initialBufLen;
                debug.write(DPDU, "have "+optionalLength+" bytes left.");
                ByteBuffer optionalBody =
                    buffer.removeBuffer(optionalLength);
     ...
     * complete PDU.
     */
    public boolean isValid()         { return getValid() == VALID_ALL;  }

    /**
     * Returns if the parsing of the binary PDU data ended before parsing
     * of the header.
     */
    public boolean isInvalid()       { return getValid() == VALID_NONE; }

    /** Returns if at least the header of the binary PDU data was correct. */
    public boolean isHeaderValid()   { return getValid() >= VALID_HEADER; }
    ...
    throws  NotEnoughDataInByteBufferException,
            UnexpectedOptionalParameterException,
            TLVException
    {
        short tag;
        short length;
    ...
    public boolean isOk() { return getCommandStatus() == Data.ESME_ROK; }
     ...
     * @see com.logica.smpp.Session#checkResponse(PDU,Response)
     */
    public boolean isGNack() { return getCommandId() == Data.GENERIC_NACK; }
    ...
    throws HeaderIncompleteException,
    ...
    public String debugString()
    {
        ...
        dbgs += Integer.toString(getCommandLength()); dbgs += " ";
        dbgs += Integer.toHexString(getCommandId()); dbgs += " ";
        dbgs += Integer.toHexString(getCommandStatus()); dbgs += " ";
    ...
    public void setApplicationSpecificInfo(Object key, Object value)
    {
        ...
        debug.write(DPDU,"setting app spec info key=\""+
    ...
    public void setApplicationSpecificInfo(Dictionary applicationSpecificInfo)
    {
        this.applicationSpecificInfo =
            cloneApplicationSpecificInfo(applicationSpecificInfo);
    }
    ...
    public Object getApplicationSpecificInfo(Object key)
    {
        ...
        debug.write(DPDU,"getting app spec info key=\""+
                    key+"\" value=\""+(value==null?"null":value)+"\"");
        return value;
    }
