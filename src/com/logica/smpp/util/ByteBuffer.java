...
/**
 * Data buffer for queued writting and reading of binary data. Provides methods
 * for appending several data types to the end of the buffer and removing them
 * from the begenning of the buffer. Used for constructing and parsing binary
 * PDUs.
 *
 ...
 * appending one byte less than it had to; it was also appending terminationg
 * zero which it hadn't to :-) 13-07-01 ticp@logica.com added getHexDump()
 * function which returns the content of the buffer in hex as String 26-09-01
 * ticp@logica.com fixed getHexDump - hi byte masked with 0x0f to mask-out the
 * sign which got extended to the hi byte after shift right 27-09-01
 * ticp@logica.com added method for appending first count bytes from byte[]
 * 15-11-01 ticp@logica.com added support for appending strings with explicitly
 * stated character encoding 20-11-01 ticp@logica.com fixed bug in appendString0
 * where if param string was null, NullPointerException was thrown
 */
